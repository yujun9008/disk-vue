function watermark(settings) {
  ...
  var page_height = 0
  //获取页面最大宽度
  var page_width = Math.max(document.body.clientWidth) - 20;
  // var cutWidth = page_width * 0.0150;
  // var page_width = page_width - cutWidth;
  //获取页面最大高度
  var page_height = Math.max(document.body.clientHeight) - 100;
  ...
  defaultSettings.watermark_cols = 3;
  // defaultSettings.watermark_x_space = parseInt((page_width - defaultSettings.watermark_x - defaultSettings.watermark_width * defaultSettings.watermark_cols) / (defaultSettings.watermark_cols - 1));
  defaultSettings.watermark_x_space = parseInt((page_width - defaultSettings.watermark_x - defaultSettings.watermark_width * 3) / 2);
  // }
  ...
  window.onresize = () => {
      ...
      var page_height = 0
      //获取页面最大宽度
      page_width = Math.max(document.body.clientWidth) - 20;
      //获取页面最大高度
      page_height = Math.max(document.body.clientHeight) - 100;
      ...
      // 清空之前的水印
      const divs = document.body.childNodes
      let arr = []

      for (let i = 0; i < divs.length; i++) {

          if (divs[i].id != '' && divs[i].id != null && divs[i].id != undefined && divs[i].id.indexOf('mask_div') != -1) {
              arr.push(divs[i].id)
          }
      }

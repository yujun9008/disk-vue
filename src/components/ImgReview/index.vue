...
export default {
  ...
  methods: {
    //  关闭图片预览
    closeImgReview() {
      this.$emit('imgReviewData', null, false);
      this.rotate = 0;
      this.zoomX = 1;
      this.zoomY = 1;
      if (this.$refs.rotate) {
  ...
  computed: {
    src: function () {
      return process.env.VUE_APP_BASE_API + "/file/preview?uri=" + this.imgReview.fileUrl + "&name=" + this.imgReview.name
    }
  }
}
</script>
...
.opera-btn-group .download-link {
  color: inherit;
}

.opera-btn-group .opera-icon {
  margin: 0 10px;
}
